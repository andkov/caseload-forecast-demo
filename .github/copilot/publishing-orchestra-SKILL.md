...
description: "**WORKFLOW SKILL** — Publish a reproducible analytics repository as a static Quarto website using a multi-agent pipeline (Orchestrator → Prompt Engineer → Editor → Publisher). USE FOR: building project websites from repo content; creating new frontend workspaces; understanding the publishing workflow; resolving publisher build errors. DO NOT USE FOR: modifying analysis code; running data pipelines; general R/Python coding. INVOKES: Publishing Orchestrator agent as entry point, which dispatches PE, Editor, and Publisher subagents."
---
...
Human ↔ Publishing Orchestrator
              ...
              ├── Publishing Editor    →  content/ + publisher.prompt.md
              └── Publishing Publisher →  _quarto.yml + _site/
```
...
| **Publishing Publisher** | Quarto scaffolding, rendering, reconciliation | No | `@publishing-publisher` |
...
| `questions.prompt.md` | Publisher | Orchestrator | Build blockers requiring human input |
...
4. **Editor phase**: Reads `editor.prompt.md`, resolves files, assembles `content/`, produces `publisher.prompt.md`. **Human checkpoint.**
5. **Publisher phase**: Reads spec + content, scaffolds Quarto project, renders `_site/`. **Human checkpoint.**
6. **Error loop**: If Publisher writes `questions.prompt.md`, Orchestrator routes questions to human, resolves, and re-runs Publisher.
...
- `.github/agents/publishing-publisher.agent.md`
...
- `.github/templates/publisher-prompt-template.md` — Schema for `publisher.prompt.md`
...
5. **Quarto-only publisher** — The architecture supports future format publishers, but only Quarto is implemented.
6. **Deterministic Publisher** — Given the same `publisher.prompt.md` and `content/`, always produces the same `_site/`.
